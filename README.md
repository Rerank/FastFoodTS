...
### Что тесты подсветили в коде

Побочный результат — список мест, где разметке не хватает семантики (обнаружился не при ревью,
а при попытке написать проверку):

- `OrdersPage` показывает ошибку простым текстом вместо `ErrorState`;
- скелетоны загрузки не имеют `role="status"` — ни тест, ни скринридер не могут понять, что идёт
  загрузка;
- итоговые суммы в корзине получили `role="group"` с явным именем — до этого значение нельзя было
  отличить от цены отдельного товара.
